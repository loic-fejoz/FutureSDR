...
pub struct LeftRightBalanceInterleaved
{
    gain_l: f32,
    gain_r: f32,
}

impl LeftRightBalanceInterleaved
{
    pub fn new(gain_l: f32, gain_r: f32) -> Block {
        Block::new_sync(
            BlockMetaBuilder::new("LeftRightBalanceInterleaved").build(),
            StreamIoBuilder::new()
                .add_input("in", mem::size_of::<f32>())
                .add_output("out", mem::size_of::<f32>())
                .build(),
            MessageIoBuilder::<LeftRightBalanceInterleaved>::new().build(),
            LeftRightBalanceInterleaved {gain_l, gain_r},
        )
    }
}

impl SyncKernel for LeftRightBalanceInterleaved
{
    fn work(
        &mut self,
        io: &mut WorkIo,
